#!/usr/bin/env bash
# One command to issue and verify a receipt for the trading-agent example.
...
zkml-trail setup model.onnx --name "trading-agent" --task "classification" --labels "sell,hold,buy" --calibration sample_input.json
